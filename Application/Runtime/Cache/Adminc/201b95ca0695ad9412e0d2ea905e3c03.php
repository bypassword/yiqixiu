...
ctrl.prototype = {
	del:function(id){
		...
		if( confirm("是否删除<?php echo ($ts['app']['app_alias']); ?>？") ){
		  $.post( '/adminc.php?s=/Group&act=delete_album',{id:id},function(text ){
			  if( text == 1 ){
	...
	edit:function(id,act){
	  if( act == 'recommend' ){
	    ...
	    $.post('/adminc.php?s=/Group&act=doChangeIsHot',{id:id,type:act},function( text ){
			  if( text == 1 ){
			  ui.success( "操作成功" );
          ...
          <table class="table table-striped table-bordered table-advance table-hover">
           <thead>
                ...
                <th align="left" scope="col"><i class="icon-bookmark"></i>套餐状态</th>
															...
															<tbody>
            <?php if(is_array($group)): foreach($group as $key=>$user): ?><tr>
				<td style="width:40px;">
                  <?php echo ($user["id"]); ?>
                </td>
                <span  class="pull-right"> <td align="center" style="width:200px;"><a href="/adminc.php?c=Group&a=e&id=<?php echo ($user["id"]); ?>"class="btn blue"> <i class=" icon-edit"> </i> 编辑 </a><a href="/adminc.php?c=Group&a=del&id=<?php echo ($user["id"]); ?>&group=group"class="btn"><i class="icon-remove icon-white"></i> 删除</a></span></h4>
                <td>  <?php echo ($user["name"]); ?></td>
              <td>  <?php echo ($user["price"]); ?></td>
                <td>  <?php echo ($user["cj_card_num"]); ?></td>
